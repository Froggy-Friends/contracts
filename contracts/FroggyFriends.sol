    ...
    ONFT721Upgradeable
{
     ...
     */
    function hibernate(
        HibernationStatus _hibernationStatus
    ) external checkHibernationIsAvailable(_hibernationStatus) {
        if (balanceOf(msg.sender) == 0 || hibernationOneDate[msg.sender] > 0)
            revert InvalidSize();
     ...
     */
    function setTadpoleReward(
        HibernationStatus _status,
        uint256 _amount
    ) external onlyOwner {
     ...
     */
    function setHibernationAvailable(
        bool _thirdyDayAvailable,
        bool _sixtyDayAvailable,
        bool _ninetyDayAvailable
    ) external onlyOwner {
        hibernationAvailable[
            HibernationStatus.THIRTY_DAYS
        ] = _thirdyDayAvailable;
        hibernationAvailable[HibernationStatus.SIXTY_DAYS] = _sixtyDayAvailable;
        hibernationAvailable[
            HibernationStatus.NINETY_DAYS
        ] = _ninetyDayAvailable;
    }
}
