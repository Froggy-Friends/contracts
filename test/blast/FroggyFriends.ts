...
import { FroggyFriends as FroggyFriendsBlast } from "../../types/contracts/blast";
...
describe("ONFT721: ", function () {
  ...
  let owner: SignerWithAddress,
    ...
    FroggyFriendsBlastFactory: ContractFactory,
    FroggyFriendsEth: FroggyFriendsEth,
    FroggyFriendsBlast: FroggyFriendsBlast;
  ...
  before(async function () {
    ...
    FroggyFriendsBlastFactory = await ethers.getContractFactory(
      "contracts/blast/FroggyFriends.sol:FroggyFriends"
    );
  });
  ...
  beforeEach(async function () {
    ...
    FroggyFriendsBlast = (await upgrades.deployProxy(
      FroggyFriendsBlastFactory,
      [minGasToStore, lzEndpointMockB.address]
    )) as FroggyFriendsBlast;

    // wire the lz endpoints to guide msgs back and forth
    lzEndpointMockA.setDestLzEndpoint(
      FroggyFriendsBlast.address,
    ...
    await FroggyFriendsEth.setTrustedRemote(
      ...
      ethers.utils.solidityPack(
        ["address", "address"],
        [FroggyFriendsBlast.address, FroggyFriendsEth.address]
      )
    );
    await FroggyFriendsBlast.setTrustedRemote(
      chainId_A,
      ethers.utils.solidityPack(
        ["address", "address"],
        [FroggyFriendsEth.address, FroggyFriendsBlast.address]
      )
    );

    // set batch size limit
    await FroggyFriendsEth.setDstChainIdToBatchLimit(chainId_B, batchSizeLimit);
    await FroggyFriendsBlast.setDstChainIdToBatchLimit(
      chainId_A,
      batchSizeLimit
    );
    ...
    await FroggyFriendsBlast.setMinDstGas(chainId_A, 1, 150000);
  });
  ...
  it("sendFrom() - your own tokens", async function () {
    ...
    await expect(FroggyFriendsBlast.ownerOf(tokenId)).to.be.revertedWith(
    ...
    expect(await FroggyFriendsBlast.ownerOf(tokenId)).to.be.equal(
      warlock.address
    );

    // estimate nativeFees
    nativeFee = (
      await FroggyFriendsBlast.estimateSendFee(
        chainId_A,
        warlock.address,
        tokenId,
        false,
    ...
    await FroggyFriendsBlast.connect(warlock).sendFrom(
      warlock.address,
      chainId_A,
      warlock.address,
      tokenId,
  ...
  it("sendFrom() - totalSupply is incremented", async function () {
    ...
    await expect(FroggyFriendsBlast.ownerOf(tokenId2)).to.be.revertedWith(
      "ERC721: invalid token ID"
    );
    await expect(FroggyFriendsBlast.ownerOf(tokenId3)).to.be.revertedWith(
      "ERC721: invalid token ID"
    );
    ...
    expect(await FroggyFriendsBlast.ownerOf(tokenId2)).to.be.equal(
      owner.address
    );
    expect(await FroggyFriendsBlast.ownerOf(tokenId3)).to.be.equal(
      owner.address
    );

    // total supply increases on dst chain
    expect(await FroggyFriendsBlast.totalSupply()).to.be.equal(3);
  });
  ...
  it("sendFrom() - reverts if not owner on non proxy chain", async function () {
    ...
    await expect(
      FroggyFriendsBlast.connect(warlock).sendFrom(
        warlock.address,
        chainId_A,
        warlock.address,
        tokenId,
  ...
  it("sendFrom() - on behalf of other user", async function () {
    ...
    await FroggyFriendsBlast.approve(warlock.address, tokenId);
  ...
  it("sendFrom() - reverts if contract is approved, but not the sending user", async function () {
    ...
    await FroggyFriendsBlast.approve(FroggyFriendsBlast.address, tokenId);
  ...
  it("sendBatchFrom()", async function () {
    ...
    await FroggyFriendsBlast.setMinGasToTransferAndStore(400000);
    ...
    await expect(FroggyFriendsBlast.clearCredits(payload))
      .to.emit(FroggyFriendsBlast, "CreditCleared")
    ...
    await expect(FroggyFriendsBlast.clearCredits(payload)).to.be.revertedWith(
  ...
  it("sendBatchFrom() - large batch", async function () {
    ...
    // clear the rest of the credits
    await (await FroggyFriendsBlast.clearCredits(payload)).wait();
